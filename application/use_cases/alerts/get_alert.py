...
class GetAlertUseCase:
    ...
    def execute_all(self, page_size: int, page: int, alert_type: Optional[AlertType] = None, status: Optional[AlertStatus] = None, sort_field: Optional[str] = None, sort_order: Optional[str] = None) -> List[Alert]:
        """Obtener todas las alertas con filtros"""
        return self.repository.get_all(page_size=page_size, page=page, alert_type=alert_type, status=status, sort_field=sort_field, sort_order=sort_order)
