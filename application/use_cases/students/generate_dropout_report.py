...
class GenerateDropoutReportUseCase:
    REQUIRED_COLUMNS = ["id", "name", "lastname", "email", "semester", "internet_home", "has_laptop", "is_female", "age", "worked_last_week", "household_size", "prev_school_public", "finished_prev_level", "repeated_subjects", "work_hours"]

    def __init__(self, prediction_service: PredictionService, student_repository: StudentRepository, alert_repository: AlertRepository, notification_service: NotificationService):
    ...
    async def execute(self, file_content: bytes) -> List[StudentRiskReportResponseDTO]:
        try:
            ...
            raise ValueError(f"No se pudo leer el archivo CSV: {str(e)}") from e
        ...
        for _, row in df.iterrows():
            try:
                ...
                if not self.student_repository.exists(student_id):
                    missing_students.append({"id": student_id, "name": str(row.get("name", "")), "lastname": str(row.get("lastname", "")), "email": str(row.get("email", "")), "semester": int(row.get("semester", 1))})
                    continue
                ...
                student_report = StudentRiskReportResponseDTO(
                    id=str(row.get("id", "N/A")), name=str(row.get("name", "Desconocido")), lastname=str(row.get("lastname", "")), email=str(row.get("email", "sin_email@ejemplo.com")), semester=int(row.get("semester", 1)), risk_status=risk_level, probability=round(probability, 4)
                )
                ...
                if risk_level != "Bajo":
                    alert_dto = CreateAlertDTO(
                        ...
                        message=(f"Hola {row.get('name', '').strip()},\n\nSe ha detectado un nivel de riesgo *{risk_level}*.\nProbabilidad estimada: {round(probability, 4)}.\n\nTe recomendamos acudir al área de apoyo académico."),
        ...
        return {"processed": processed_students, "not_found": missing_students}
