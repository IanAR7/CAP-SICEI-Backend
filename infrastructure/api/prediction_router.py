...
def get_report_use_case(prediction_service=Depends(get_prediction_service), student_repo=Depends(get_student_repository), alert_repo=Depends(get_alert_repository), notification_service=Depends(get_notification_service)):
    return GenerateDropoutReportUseCase(prediction_service=prediction_service, student_repository=student_repo, alert_repository=alert_repo, notification_service=notification_service)


@router.post("/dropout-risk", response_model=RiskPrediction, status_code=status.HTTP_200_OK)
async def predict_student_dropout(data: StudentFeatures, use_case: PredictDropoutUseCase = Depends(get_predict_use_case)):
    try:
        return use_case.execute(data)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error processing prediction: {str(e)}") from None


@router.post("/train", response_model=TrainingStatus, status_code=status.HTTP_200_OK)
async def train_model():
    try:
        return TrainingStatus(success=True, message="Entrenamiento iniciado correctamente. El modelo se actualizará en breve.", accuracy=0.0)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error initiating training: {str(e)}") from None


@router.post("/batch-upload")
async def upload_students_csv(file: UploadFile = File(...), use_case: GenerateDropoutReportUseCase = Depends(get_report_use_case)) -> Dict[str, Any]:
    if not file.filename.endswith(".csv"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="El archivo debe ser un CSV (.csv)")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error procesando el archivo CSV: {str(e)}") from None
