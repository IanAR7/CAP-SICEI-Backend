...
from infrastructure.schemas.professor_schema import CreateProfessorDTO, ProfessorResponseDTO, UpdateProfessorDTO

router = APIRouter(prefix="/professors", tags=["Professors"])


@router.post("/", status_code=status.HTTP_201_CREATED, response_model=ProfessorResponseDTO)
async def create_professor(professor_data: CreateProfessorDTO, db: Session = Depends(get_db)) -> ProfessorResponseDTO:
    try:
        repo = ProfessorRepositoryImpl(db)
        use_case = CreateProfessorUseCase(repo)
        professor = use_case.execute(map_create_professor_dto_to_entity(professor_data))
...
async def get_professor_by_id(professor_id: str, db: Session = Depends(get_db)) -> ProfessorResponseDTO:
    try:
        repo = ProfessorRepositoryImpl(db)
...
async def get_all_professors(
    db: Session = Depends(get_db), page_size: Annotated[int, Query(alias="pageSize")] = 25, current: Annotated[int, Query(alias="current")] = 1, sort_field: Optional[str] = Query(default=None, alias="sorters[0][field]"), sort_order: Optional[str] = Query(default=None, alias="sorters[0][order]")
) -> List[ProfessorResponseDTO]:
    try:
        repo = ProfessorRepositoryImpl(db)
        use_case = GetProfessorUseCase(repo)
        professors = use_case.execute_all(page_size=page_size, page=current, sort_field=sort_field, sort_order=sort_order)
...
async def update_professor(professor_id: str, professor_data: UpdateProfessorDTO, db: Session = Depends(get_db)) -> ProfessorResponseDTO:
    try:
        ...
        updated_professor = use_case.execute(map_update_professor_dto_to_entity(professor_id, professor_data))
...
async def delete_professor(professor_id: str, db: Session = Depends(get_db)):
    try:
        repo = ProfessorRepositoryImpl(db)
