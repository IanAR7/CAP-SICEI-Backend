    ...
    async def send_alert(self, alert: Alert) -> bool:
        ...
        try:
            if alert.channel == NotificationChannel.EMAIL:
                html_message = self.template_service.generate_alert_email(title=alert.title, message=alert.message, alert_type=alert.alert_type.value)
                return await self.email_service.send_email(recipients=alert.target_recipients, subject=alert.title, message=html_message, html=True)

            elif alert.channel == NotificationChannel.SMS:
                return await self.sms_service.send_sms(recipients=alert.target_recipients, message=f"{alert.title}: {alert.message}")

            elif alert.channel == NotificationChannel.BOTH:
                email_sent = await self.email_service.send_email(recipients=alert.target_recipients, subject=alert.title, message=self.template_service.generate_alert_email(alert.title, alert.message, alert.alert_type.value), html=True)
                sms_sent = await self.sms_service.send_sms(recipients=alert.target_recipients, message=f"{alert.title}: {alert.message}")
