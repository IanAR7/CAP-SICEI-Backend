...
def map_alert_entity_to_model(alert: Alert) -> AlertModel:
    ...
    return AlertModel(
        ...
        status=AlertStatusEnum[alert.status.name],
        channel=NotificationChannelEnum[alert.channel.name],
...
def map_alert_model_to_entity(alert_model: AlertModel) -> Alert:
    ...
    return Alert(
        ...
        alert_type=AlertType[alert_model.alert_type.name],
        status=AlertStatus[alert_model.status.name],
        channel=NotificationChannel[alert_model.channel.name],
